...
import NeuralNetworkReachability.ForwardAlgorithms
using NeuralNetworkReachability.ForwardAlgorithms: Verisig
using LazySets: AffineMap, LazySet, Singleton, ×
using ControllerFormats: FeedforwardNetwork, Id, Sigmoid, Tanh, layers

@static if isdefined(Base, :get_extension)
    using ReachabilityAnalysis: BlackBoxContinuousSystem, IVP, TMJets, evaluate,
                                solve
    using TaylorIntegration: @taylorize
    using TaylorIntegration: TaylorIntegration, Taylor1, TaylorN  # `@taylorize` assumes that these symbols are defined
    using TaylorModels: TaylorModelN
else
    using ..ReachabilityAnalysis: BlackBoxContinuousSystem, IVP, TMJets,
                                  evaluate, solve
    using ..TaylorIntegration: @taylorize
    using ..TaylorIntegration: TaylorIntegration, Taylor1, TaylorN
    using ..TaylorModels: TaylorModelN
end
